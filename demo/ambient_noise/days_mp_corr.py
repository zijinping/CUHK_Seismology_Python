...
def day_wf_corr(args):
    """
    Do waveform cross-correlation by day
    """
    sta1 = args.sta1
    sta2 = args.sta2
    cmp1 = args.cmp1
    ...
    # Frequency band info
    Tmax = args.period_band[0,1]
    Tmin = args.period_band[0,0]
    ...
    for yr in range(year_range[0],year_range[1]+1):
        ...
        for day in range(day_range[0],day_range[1]+1):
            ...
            logger.info(f"load data sccess: {yr} {day} {sta1} {sta2}")
            if st_check(tr1) and st_check(tr2):
                ...
                if sum(np.isnan(CFcnMB))==0:  # No NaN data in CFcn
                    ...
                    CF_file = os.path.join(args.out_dir,f"{sta1}_{sta2}_{yr}_{day}.pkl")
...
if __name__ == "__main__":
    ...
    Usage:
        "python day_wf_corr.py --stations station1/station2"
    '''
